...
class AndroidPhoneAdapter(
    val context: Context,
    var onItemClickListener: AndroidListOnItemClickListener
) : RecyclerView.Adapter<AndroidPhoneAdapter.classViewHolder>() {
    ...
    interface AndroidListOnItemClickListener {
        fun AndroidOnClick(position: Int, list: ArrayList<Data>, view: View, type:String, phoneName: String)
    }

    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): AndroidPhoneAdapter.classViewHolder {
        val view = inflater.inflate(R.layout.day_deals_list, parent, false)
        return classViewHolder(view)
    }

    override fun onBindViewHolder(holder: AndroidPhoneAdapter.classViewHolder, position: Int) {
        with(holder) {
            ...
            if (dayDealsList[position].custPrice.toString().isNullOrEmpty()) {
                tvOff.text = "0 %"
            } else {
                val pricedifference = dayDealsList[position].mRP.toFloat()
                    .minus(dayDealsList[position].custPrice.toFloat())
            ...
            if (dayDealsList[position].custPrice.toString().isNullOrEmpty()) {
                ...
                tvOffPrice.text = "₹ " + dayDealsList[position].custPrice.toString()
            }
            ...
            tvSavePrice.text = "Save ₹ " + dayDealsList[position].mRP.toFloat().minus(dayDealsList[position].custPrice.toFloat())
...
//        }
            itemView.setOnClickListener {
                onItemClickListener.AndroidOnClick(position, dayDealsList, it, "Aclick", dayDealsList[position].model)

            }

            btnBuyNow.setOnClickListener {
                onItemClickListener.AndroidOnClick(position, dayDealsList, it,"Abuy", dayDealsList[position].model)

            }

            ivAddToCart.setOnClickListener {
                onItemClickListener.AndroidOnClick(position, dayDealsList, it,"Acart", dayDealsList[position].model)
            }
        }
    }
    ...
    inner class classViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        var btnBuyNow: Button
        var ivAddToCart: LinearLayout

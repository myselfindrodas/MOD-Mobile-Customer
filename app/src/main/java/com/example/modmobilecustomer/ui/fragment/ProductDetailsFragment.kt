...
import com.example.modmobilecustomer.data.model.CategoryListItemData
...
import com.example.modmobilecustomer.data.model.invoicecreatemodel.BillJSON
import com.example.modmobilecustomer.data.model.invoicecreatemodel.InvoiceDetail
...
import com.example.modmobilecustomer.data.model.pocreatemodel.OrderDetail
import com.example.modmobilecustomer.data.model.pocreatemodel.POJSON
...
import com.example.modmobilecustomer.data.model.viewaddressmodel.Addressdata
...
import com.example.modmobilecustomer.databinding.FragmentProductDetailsBinding
import com.example.modmobilecustomer.ui.MainActivity
import com.example.modmobilecustomer.ui.MainActivity.Companion.payment
import com.example.modmobilecustomer.ui.adapter.AddressAdapter
import com.example.modmobilecustomer.ui.adapter.CategoryListItemAdapter
import com.example.modmobilecustomer.ui.adapter.ColorListAdapter
import com.example.modmobilecustomer.ui.adapter.FeedBackAdapter
import com.example.modmobilecustomer.ui.adapter.RelatedProductListAdapter
import com.example.modmobilecustomer.ui.adapter.SlidingPhonesImageAdapter
import com.example.modmobilecustomer.ui.adapter.StorageAdapter
...
class ProductDetailsFragment : Fragment(), SlidingPhonesImageAdapter.OnItemClickListener,
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        try {
            ...
            val pricedifference = data?.mRP?.toFloat()!!.minus(data?.custPrice!!.toFloat())
            val offpercent = (pricedifference.div(data?.mRP?.toFloat()!!) * 100)
            binding.tvDiscount.text = String.format("%.2f", offpercent) + " %"
            binding.tvCustomerprice.text = "₹ " + data?.custPrice
...
//        }
        ...
        binding.feedbackList.layoutManager = LinearLayoutManager(mainActivity, LinearLayoutManager.HORIZONTAL, false)
...
//                                Toast.makeText(this, it.message, Toast.LENGTH_SHORT).show()
                        ...
                        Status.SUCCESS -> {
                            ...
                            if (resource.data?.response?.status.equals("true")) {
                                ...
                                if (resource.data?.response?.data?.isEmpty()!!){
                                    ...
                                    price = resource.data.response.data.get(0).custPrice.toString()
                                    productid = resource.data.response.data.get(0).imei1.toString()
                                    purchaseprice = resource.data.response.data.get(0).purchasePrice.toString()
                                    stocktype = resource.data.response.data.get(0).stockType.toString()

                                    binding.phoneName.text = resource.data.response.data.get(0)?.modelName
                                    binding.tvphoneVersion.text = resource.data?.response?.data?.get(0)?.brand
                                    binding.tvStockqty.text = resource.data?.response?.data?.get(0)?.stockQty + " In Stock"
                                    val pricedifference = resource.data.response.data.get(0).mRP.toFloat()!!.minus(resource.data?.response?.data?.get(0)?.custPrice!!.toFloat())
                                    val offpercent = (pricedifference.div(resource.data.response.data?.get(0)?.mRP?.toFloat()!!) * 100)
                                    binding.tvDiscount.text = String.format("%.2f", offpercent) + " %"
                                    binding.tvCustomerprice.text = "₹ " + resource.data?.response?.data?.get(0)?.custPrice
                                    binding.tvMRP.text = "₹ " + resource.data?.response?.data?.get(0)?.mRP
                                    binding.tvSaveAmount.text = "Save ₹ " + pricedifference
                                    binding.tvFeatures.text = resource.data.response.data.get(0).features
    ...
    override fun storageListOnClick(
        position: Int,
        list: ArrayList<com.example.modmobilecustomer.data.model.storagecolormodel.Data>,
        view: View
    ) {

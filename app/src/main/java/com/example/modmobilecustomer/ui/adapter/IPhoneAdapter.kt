...
class IPhoneAdapter(
    val context: Context,
    var onItemClickListener: IphoneListOnItemClickListener
) : RecyclerView.Adapter<IPhoneAdapter.classViewHolder>() {
    ...
    interface IphoneListOnItemClickListener {
        fun IphoneOnClick(position: Int, list: ArrayList<Data>, view: View, type:String, phoneName: String)
    }

    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): IPhoneAdapter.classViewHolder {
        val view = inflater.inflate(R.layout.day_deals_list, parent, false)
        return classViewHolder(view)
    }

    override fun onBindViewHolder(holder: IPhoneAdapter.classViewHolder, position: Int) {
        with(holder) {
...
//        }
            itemView.setOnClickListener {
                onItemClickListener.IphoneOnClick(position, dayDealsList, it, "click", dayDealsList[position].model)

            }

            btnBuyNow.setOnClickListener {
                onItemClickListener.IphoneOnClick(position, dayDealsList, it,"buy", dayDealsList[position].model)

            }

            ivAddToCart.setOnClickListener {
                onItemClickListener.IphoneOnClick(position, dayDealsList, it,"cart", dayDealsList[position].model)
            }
        }
    }

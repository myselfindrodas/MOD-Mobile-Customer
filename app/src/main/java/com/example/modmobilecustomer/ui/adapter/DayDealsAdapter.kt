...
class DayDealsAdapter(
    val context: Context,
    var onItemClickListener: DayDealsListOnItemClickListener
) : RecyclerView.Adapter<DayDealsAdapter.classViewHolder>() {
    ...
    interface DayDealsListOnItemClickListener {
        fun dayDealsOnClick(position: Int, list: ArrayList<Data>, view: View, type:String, phoneName: String)
    }

    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): DayDealsAdapter.classViewHolder {
        val view = inflater.inflate(R.layout.day_deals_list, parent, false)
        return classViewHolder(view)
    }

    override fun onBindViewHolder(holder: DayDealsAdapter.classViewHolder, position: Int) {
        with(holder) {
            ...
            tvSavePrice.text = "Save ₹ " + dayDealsList[position].mRP.toFloat()
                .minus(dayDealsList[position].custPrice.toFloat())
...
//        }
            itemView.setOnClickListener {
                onItemClickListener.dayDealsOnClick(position, dayDealsList, it, "Aclick", dayDealsList[position].model)

            }

            btnBuyNow.setOnClickListener {
                onItemClickListener.dayDealsOnClick(position, dayDealsList, it,"Abuy", dayDealsList[position].model)

            }

            ivAddToCart.setOnClickListener {
                onItemClickListener.dayDealsOnClick(position, dayDealsList, it,"Acart", dayDealsList[position].model)
            }
        }
    }

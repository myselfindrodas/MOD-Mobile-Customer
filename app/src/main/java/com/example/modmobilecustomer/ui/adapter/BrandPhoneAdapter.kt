...
class BrandPhoneAdapter(
    val context: Context,
    var onItemClickListener: BrandListOnItemClickListener
) : RecyclerView.Adapter<BrandPhoneAdapter.classViewHolder>() {
    ...
    interface BrandListOnItemClickListener {
        fun BrandOnClick(position: Int, list: ArrayList<Data>, view: View, type:String, phoneName: String)
    }

    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): BrandPhoneAdapter.classViewHolder {
        val view = inflater.inflate(R.layout.day_deals_list, parent, false)
        return classViewHolder(view)
    }

    override fun onBindViewHolder(holder: BrandPhoneAdapter.classViewHolder, position: Int) {
        with(holder) {
...
//        }
            itemView.setOnClickListener {
                onItemClickListener.BrandOnClick(position, dayDealsList, it, "click", dayDealsList[position].model)

            }

            btnBuyNow.setOnClickListener {
                onItemClickListener.BrandOnClick(position, dayDealsList, it,"buy", dayDealsList[position].model)

            }

            ivAddToCart.setOnClickListener {
                onItemClickListener.BrandOnClick(position, dayDealsList, it,"cart", dayDealsList[position].model)
            }
        }
    }
